...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        return (self.data[index].clone().detach(), self.label[index].clone().detach())
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ratio, kernel_size, stride, groups, downsample, is_first_block=False, use_bn=True, use_do=True):
        ...
        r = 2
        self.se_fc1 = nn.Linear(self.out_channels, max(self.out_channels//r, 1))
        self.se_fc2 = nn.Linear(max(self.out_channels//r, 1), self.out_channels)
        ...
        for m in self.modules():
            if isinstance(m, MyConv1dPadSame):
